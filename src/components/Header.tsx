...
const Header = () => {
  ...
  const searchSvgVariants: Variants = {
    start: { x: 0 },
    end: { x: isOpenSearch ? -250 : 0, transition: { duration: 0.5, type: "keyframes" } },
  };
  ...
  const handleToggleSearchBar = useCallback(() => {
    const targetScaleX = isOpenSearch ? 0 : 1;
    inputAnimationControls.start({ scaleX: targetScaleX, type: "keyframes", transition: { duration: 0.5 } });
    setIsOpenSearch((prev) => !prev);
  }, [inputAnimationControls, isOpenSearch]);
  ...
  return (
    <Container animate={containerAnimationControls}>
      ...
      <SearchForm onSubmit={handleSubmit(onValid)}>
        <MotionSearchInput
          {...register("keyword", { required: "키워드를 입력해주세요." })}
          type="text"
